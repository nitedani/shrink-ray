  ...
  describe('when "Accept-Encoding: br"', function () {
    ...
    it('should have a correctly encoded brotli response', function (done) {
      ...
      brotliRequest(server)
        .pipe(stream)
        .on('finish', function () {
          assert.equal('hello, world', zlib.decompressSync(stream.getContents()).toString('utf-8'));
          done();
        });
    });
    ...
    it('should apply the brotli parameters from options', function (done) {
      ...
      brotliRequest(server)
        ...
        .on('finish', function () {
          ...
          assertBuffersEqual(
            stream.getContents(),
            zlib.compressSync(Buffer.from('hello, world', 'utf-8'), {quality: 8})
          );
          done();
        });
    });
  ...
  describe('when caching is turned on', function () {
    ...
    it('should cache a brotli response with the same ETag', function (done) {
      ...
      brotliRequest(server)
        .pipe(stream)
        .on('finish', function () {
          assert.equal('hello, world #0', zlib.decompressSync(stream.getContents()).toString('utf-8'));
          const stream2 = new streamBuffers.WritableStreamBuffer();
          brotliRequest(server)
            .pipe(stream2)
            .on('finish', function () {
              assert.equal('hello, world #0', zlib.decompressSync(stream2.getContents()).toString('utf-8'));
              done();
            });
        });
    });
